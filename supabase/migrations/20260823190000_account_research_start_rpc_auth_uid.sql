-- Fix start_account_research_run signature drift: remote had 5-arg version with
-- p_requested_by; app calls 4-arg RPC using auth.uid() internally (PR2 final).

drop function if exists public.start_account_research_run(
  integer,
  text,
  text,
  uuid,
