-- Unseen Product Email engagement alerts (Line Sheet).
-- last_* updated on every open/click; product-level seen cursor clears alerts on drawer open.
...
where opened_at is not null and last_opened_at is null;

update public.system_messages
set last_clicked_at = clicked_at
where clicked_at is not null and last_clicked_at is null;

create index if not exists system_messages_last_opened_at_idx
  on public.system_messages (last_opened_at)
  where last_opened_at is not null;

create index if not exists system_messages_last_clicked_at_idx
  on public.system_messages (last_clicked_at)
  where last_clicked_at is not null;

create table if not exists public.product_outreach_engagement_seen (
  catalog_item_id uuid primary key references public.catalog_items (id) on delete cascade,
  seen_at timestamptz not null default now()
);

alter table public.product_outreach_engagement_seen enable row level security;

drop policy if exists "approved staff full access" on public.product_outreach_engagement_seen;
create policy "approved staff full access" on public.product_outreach_engagement_seen
  for all to authenticated
  using (public.is_approved_staff())
  with check (public.is_approved_staff());

-- Refresh webhook apply RPC to set last_opened_at / last_clicked_at on engagement.
