-- Monotonic last_opened_at / last_clicked_at + system receipt time for unseen alerts.
-- Delayed/out-of-order Resend webhooks must not regress provider occurrence timestamps;
-- unseen alerts key off when the engagement was successfully applied locally.
...
where last_engagement_received_at is null
  and (last_opened_at is not null or last_clicked_at is not null);

create index if not exists system_messages_last_engagement_received_at_idx
  on public.system_messages (last_engagement_received_at)
