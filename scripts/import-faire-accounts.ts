...
if (!OWNER_USER_ID) {
  console.error('Need OUTREACH_PREP_ACTOR_USER_ID');
  process.exit(1);
}
...
function proposeColumnMap(headers: string[]): Record<string, string> {
  ...
  for (const [field, aliases] of Object.entries(ALIASES)) {
    const hit = normalized.find(
      (h) => !used.has(h.raw) && aliases.some((a) => h.key === a || h.key.startsWith(`${a} `)),
    );
    if (hit) {
      map[field] = hit.raw;
      used.add(hit.raw);
...
function normalizeRow(
  ...
  return {
    ...
    email,
    storeTypeRaw,
    addressUncertain: !street || !postalCode,
    warnings,
  };
}
...
try {
  ...
  console.error(`Could not read ${xlsxPath}`);
  process.exit(1);
}
...
if (!colMap.businessName) {
  console.error('ERROR: businessName not mapped');
  process.exit(1);
}
...
for (const row of normalized) {
  const flags = [
    ...
    .join(', ');
  console.log(
    `  Row ${row.rowNumber}: ${row.name} | ${row.city ?? '??'}, ${flags} | email: ${row.email ?? 'none'}`,
  );
}
...
if (!ogrLine) {
  console.error('OGR line not found');
  process.exit(1);
}
...
const { data: prospects } = await client
  .from('prospects')
  .select('id, name, city, territory_id, account_status, external_id');
...
const { count: openedCount } = await client
...
function matchRow(row: NormalizedRow): MatchedRow {
  ...
  const geoHits = thinProspects.filter((p) => {
    ...
    if (row.city && p.city.trim().toLowerCase() === (row.city ?? '').trim().toLowerCase())
      return true;
    return false;
  });
  if (geoHits.length === 1) {
    return {
      ...
      matchId: geoHits[0].id,
      matchName: geoHits[0].name,
      reasons,
    };
  }
  if (geoHits.length > 1) {
    return {
      ...
      matchId: null,
      matchName: null,
      reasons: ['Multiple geo+name hits'],
    };
  }

  // Name-only match in OR/WA
  const nameHits = thinProspects.filter(
    (p) =>
      normalizeProspectName(p.name) === nameNorm && (p.stateCode === 'or' || p.stateCode === 'wa'),
  );
  if (nameHits.length === 1) {
    return {
      ...row,
      decision: 'link_existing',
      matchId: nameHits[0].id,
      matchName: nameHits[0].name,
      reasons,
    };
  }
  if (nameHits.length > 1) {
    return {
      ...
      matchId: null,
      matchName: null,
      reasons: ['Multiple name hits in OR/WA'],
    };
  }
...
const eligibleRows = matchedRows.filter(
  (r) => r.decision === 'create_retailer' || r.decision === 'link_existing',
);

// Load OGR SLT assignments for territory linking
const { data: sltRows } = await client
...
if (batchErr) {
  ...
  if (
    eb &&
    ['committed', 'enriching', 'enrichment_partial', 'completed'].includes(eb.status as string)
  ) {
    console.log(`Batch already committed: ${eb.id} (${eb.status})`);
    process.exit(0);
  }
  if (!eb) {
    console.error('Could not create import batch:', batchErr.message);
    process.exit(1);
  }
    ...
    .upsert(
      {
        ...
        raw_address_text: [row.street, row.city, row.stateCode?.toUpperCase(), row.postalCode]
          .filter(Boolean)
          .join(', '),
      },
...
const { data: allPersisted } = await client
  .from('account_import_rows')
...
for (const row of eligibleRows) {
  ...
  if (!persisted) {
    console.error(`  Row ${row.rowNumber}: no persisted row`);
    failed++;
    continue;
  }
  ...
  if (!terrId) {
    console.error(`  Row ${row.rowNumber}: no territory for state ${row.stateCode}`);
    failed++;
    continue;
  }
  const sltId = sltByTerrId.get(terrId) ?? null;

  const addressStr = [row.street, row.city, row.stateCode?.toUpperCase(), row.postalCode]
    .filter(Boolean)
    .join(', ');
  ...
  if (row.decision === 'create_retailer') {
    payload = {
      ...
      field_changes: [
        ...
        ...(row.postalCode
          ? [{ field_path: 'postal_code', old_value: null, new_value: row.postalCode }]
          : []),
      ],
  ...
  const { data: rpcData, error: rpcError } = await client.rpc(
    'commit_account_import_row' as never,
    {
  ...
  if (rpcError) {
    if (rpcError.message.includes('Forbidden')) {
      console.error(
        `  Row ${row.rowNumber} (${row.name}): ⛔ Forbidden — commit_account_import_row requires owner JWT.`,
      );
...
await client
  .from('account_import_batches')
  .update({ status: committed > 0 ? 'committed' : 'previewed' })
  .eq('id', batchId);

const { count: newActive } = await client
  .from('prospects')
  .select('*', { count: 'exact', head: true })
  .eq('account_status', 'active_account');
const { count: newOpened } = await client
  .from('retailer_line_accounts')
  .select('*', { count: 'exact', head: true })
  .eq('sales_line_id', salesLineId)
  .eq('relationship_status', 'opened');
