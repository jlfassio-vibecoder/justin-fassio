...
| G17 | `format:check` not in CI | Style drift possible |
| G18 | Security headers lack CSP / HSTS | Baseline hardening incomplete |
...
15. `format:check` in CI; optional Playwright smoke (login → `/app` → pending/approved).  
16. CSP + HSTS on Vercel; tighten Auth redirect allow-list for previews.
...
- [ ] Server surface exists for any agent/tooling that needs secrets  

**Phase-gate conclusion:** Safe to start the next feature branch for planning and scaffolding, but **do not** build agent tools that assume CRM persistence or confidential directory protection until P0–P1 (and ideally P2 item 9) are addressed.
...
*Generated as a point-in-time architectural assessment. Re-run after persistence or hosting-mode changes.*
