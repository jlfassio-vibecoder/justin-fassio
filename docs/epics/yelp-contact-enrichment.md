...
**Status:** Planning (Aug 2026) — do not implement from this doc without an approved PR plan.  
...
| Behavior | Exa/Perplexity snippets citing public pages; **null if not explicit** |
| LinkedIn | Search-result snippets only — never fetch authenticated profiles |
...
- [ ] `YELP_FUSION_API_KEY` documented in `.env.example`
- [ ] `src/lib/yelp/businessMatch.ts` + unit tests
- [ ] `scripts/yelp-enrich-oregon-prospects.ts` dry-run on 5 Oregon Coast prospects
- [ ] Manual review of pilot report
- [ ] Optional `--apply` with audit rows
...
| 5 | LinkedIn in Exa queries | Role-verification queries only; never as website candidate |
