...
import { runSilenceCheck } from '@/lib/liveChat';
...
export const POST: APIRoute = async ({ request }) => {
  const limited = checkChatRateLimit(`chat-silence:${clientKey(request)}`);
  ...
  const result = await runSilenceCheck(admin, { threadId, userId: gate.userId });
  if (!result.ok) return json({ ok: false, error: result.error }, 400);

  return json({
    ok: true,
    inserted: result.inserted,
    wittyLine: result.wittyLine ?? null,
