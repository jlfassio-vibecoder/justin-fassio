...
import { startBatchEnrichment } from '@/lib/accountImport/enrich';
import {
  jsonAccountImport,
  parseRetailerIds,
...
export const POST: APIRoute = async ({ request, params }) => {
  ...
  const result = await startBatchEnrichment(gate.supabase, gate.userId, {
    salesLineId: gate.salesLineId,
    batchId: gate.batchId,
    retailerIds: parseRetailerIds(body.retailer_ids),
    failedOnly: body.failed_only === true,
  });
