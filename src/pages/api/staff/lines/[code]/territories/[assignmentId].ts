...
async function loadLine(
  client: Parameters<typeof updateSalesLineTerritory>[0],
...
export const PATCH: APIRoute = async ({ params, request }) => {
  ...
  if (!assignmentId) {
    return json({ ok: false, error: TERRITORY_ADMIN_ERRORS.unknownAssignment }, 400);
  }
  ...
  const result = await updateSalesLineTerritory(
    gate.supabase,
    loaded.line,
    assignmentId,
    parseWriteBody(body),
...
export const DELETE: APIRoute = async ({ params, request }) => {
  ...
  const result = await deleteSalesLineTerritory(gate.supabase, loaded.line, assignmentId);
  if (!result.ok) return json({ ok: false, error: result.error }, result.status);
  return json({ ok: true });
};
