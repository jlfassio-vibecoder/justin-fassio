...
- **Vercel AI SDK baseline:** On-demand route [`src/pages/api/agent.ts`](src/pages/api/agent.ts) (`prerender = false`) streams via `streamText` + AI Gateway model strings (`openai/gpt-4o`). Approved-staff JWT required. UI: multi-turn **AI assist** (`useChat` + `toUIMessageStreamResponse`) in `/app` chrome ([`AIAssistantModal`](src/components/ui/AIAssistantModal.tsx)), with Prospects **Ask AI**, Log Call success prefills, Insights tag click, and Calls **Coach** for objection talk tracks (shared [`objectionCatalog`](src/lib/objectionCatalog.ts)). Phase I tools (`getProspectSummary`, `listRecentCalls`) read prospects/calls under the caller’s JWT + RLS (no OpenAI secrets on Vercel for tools). Keep Edge `suggest-follow-ups` for the Prospects Suggest button. Local Gateway auth: `vercel link` + `vercel env pull`, or set `AI_GATEWAY_API_KEY` in `.env.local` (gitignored via `.env.*`).
